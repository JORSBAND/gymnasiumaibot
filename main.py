...
def _load_service_account_from_env_or_file() -> dict | None:
    ...
    Robust service account loader:
      ...
      3) fallback to the local file ./gymnasiumaibot-firebase-adminsdk-fbsvc-4088361461.json if present
    ...
    if env_val:
        ...
        if env_strip.startswith("{"):
            ...
            # maybe it's a base64-encoded JSON or a file path
            # try base64 decode safely
            try:
    ...
    # fallback: check repo local file (useful for testing / when you uploaded the json to the app)
    local_fallback = "./gymnasiumaibot-firebase-adminsdk-fbsvc-4088361461.json"
    try:
...
try:
    ...
    if sa_dict is None:
        raise RuntimeError("No Firebase credentials found: set FIREBASE_CREDENTIALS env var (JSON, base64 or path) or place service account JSON at ./gymnasiumaibot-firebase-adminsdk-fbsvc-4088361461.json")

    # Ensure private_key has real newlines if it was stored with escaped \n
    if isinstance(sa_dict.get("private_key"), str):
        sa_dict["private_key"] = sa_dict["private_key"].replace("\\\\n", "\n")

    # Create credential and initialize (credentials, firestore, initialize_app are imported earlier in file)
    cred = credentials.Certificate(sa_dict)
    try:
        initialize_app(cred)
    except Exception as e_init:
        # firebase_admin raises an error if already initialized; ignore that safely
